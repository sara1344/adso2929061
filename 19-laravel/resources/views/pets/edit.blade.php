        ...
        <div class="card text-white md:w-[720px] w-[320px] bg-black/20 p-4 mb-4 rounded">
            <form method="POST" action="{{ url('users/'.$user->id) }}" class="flex flex-col md:flex-row gap-4 mt-4" enctype="multipart/form-data">
                ...
                <div class="w-full md:w-[320px]">
                    ...
                    <input type="text" class="input bg-[#0009] outline-0" name="name" placeholder="Buddy" value="{{ old('name', $user->name) }}">
                    @error('name')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    {{-- Kind --}}
                    <label class="label text-white">Kind:</label>
                    <input type="text" class="input bg-[#0009] outline-0" name="kind" placeholder="Golden Retriever" value="{{ old('kind', $user->kind) }}">
                    @error('kind')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    {{-- Weight --}}
                    <label class="label text-white">Weight:</label>
                    <input type="text" class="input bg-[#0009] outline-0" name="weight" placeholder="10Kg" value="{{ old('weight', $user->weight) }}">
                    @error('weight')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    {{-- Age --}}
                    <label class="label text-white">Age:</label>
                    <input type="text" class="input bg-[#0009] outline-0" name="age" placeholder="10" value="{{ old('age', $user->age) }}">
                    @error('age')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    {{-- Breed --}}
                    <label class="label text-white">Breed:</label>
                    <input type="text" class="input bg-[#0009] outline-0" name="breed" placeholder="Golden Retriever" value="{{ old('breed', $user->breed) }}">
                    @error('breed')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    {{-- Gender --}}
                ...
                <div class="w-full md:w-[320px]">
                    ...
                    <input type="text" class="input bg-[#0009] outline-0" name="location" placeholder="City, State" value="{{ old('location', $user->location) }}">
                    @error('location')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    {{-- Description --}}
                    <label class="label text-white">Description:</label>
                    <textarea class="textarea bg-[#0009] outline-0" name="description" placeholder="Describe your pet...">{{ old('description', $user->description) }}</textarea>
                    @error('description')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    {{-- active --}}
                    ...
                    <select name="active" class="select bg-[#0009] outline-0">
                        <option value="">Select...</option>
                        <option value="1" @if(old('active', $user->active) == '1') selected @endif>Yes</option>
                        <option value="0" @if(old('active', $user->active) == '0') selected @endif>No</option>
                    </select>
                    @error('active')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    ...
                    <select name="status" class="select bg-[#0009] outline-0">
                        <option value="">Select...</option>
